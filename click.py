...
def get_all_download_links(driver, url):
    ...
    driver.get(url)
    matches = re.findall(r'(?<=src=\").+.jpg(?=\")', driver.page_source)
    return matches
...
class Inception:
    ...
    def print_scores(self, pred, k=10, only_first_name=True):
        ...
        idx = pred.argsort()
        a= driver.find_element_by_id("de")
        pc=a.text
        desc=pc.lower()
...
#         print(lists)
        for y in lists:
                human_string = y
                new_label = model.get_key(human_string)
                if desc == new_label:
                    print(new_label)
...
if __name__ == '__main__':
    ...
    Labels = {
            'bicycle':['bike','bicycle','off-roader','cycle'],
            'chair':['couch','chair','fluffy'],
            'fireworks':['volcano','fountain','sea urchin','cracker'],
            'guitar':['guitar','banjo','electric guitar','acoustic guitar'],
            'lion':['lion','king of Beasts','Panthra Leo'],
            'tiger':['tiger','Panthra Tigress'],
            'waterfall':['dam','valley','vale','cliff','drop-off'],
            'wristwatch':['clock','watch'],
            'cat':['cat','Egyptian cat','Siamese','Felischaus','manul','Manx','Angora','Felis'],    
            'dog':['dog','domestic dog','pooch','doggie','doggy','barker','bow-wow','hunting dog','sporting dog','gun dog','pointer','Spanish pointer','vizsla','Hungarian pointer','German short-haired pointer','setter','English setter','Gordon setter','Irish setter','red setter', 
           'bird dog','spaniel','field spaniel','water spaniel','American water spaniel','Irish water spaniel','springer spaniel, springer','Welsh springer spaniel','English springer','English springer spaniel','clumber','clumber spaniel','Brittany spaniel','cocker spaniel', 
           'English cocker spaniel','cocker','Sussex spaniel','griffon','wire-haired pointing griffon','water dog','retriever','golden retriever','Chesapeake Bay retriever','curly-coated retriever','flat-coated retriever','Labrador retriever','dachshund','dachsie','badger dog', 
           'sausage dog','sausage hound','terrier','Australian terrier','fox terrier','smooth-haired fox terrier','wire-haired fox terrier','soft-coated wheaten terrier','wirehair','wirehaired terrier','wire-haired terrier','Welsh terrier','Sealyham terrier','Sealyham', 
           'Lakeland terrier','Dandie Dinmont','Dandie Dinmont terrier','Airedale','Airedale terrier','Lhasa','Lhasa apso','Yorkshire terrier','West Highland white terrier','schnauzer','giant schnauzer','miniature schnauzer','standard schnauzer','bullterrier, bull terrier', 
           'Staffordshire bullterrier','Staffordshire bull terrier','American Staffordshire terrier','Staffordshire terrier','American pit bull terrier','pit bull terrier','Norfolk terrier','cairn','cairn terrier','Kerry blue terrier','Scotch terrier','Scottish terrier','Scottie', 
           'Boston bull','Boston terrier','Bedlington terrier','Irish terrier','silky terrier','Sydney silky','Skye terrier','Clydesdale terrier','Norwich terrier','Tibetan terrier','chrysanthemum dog','rat terrier','ratter','Manchester terrier','black-and-tan terrier', 
           'toy Manchester','toy Manchester terrier','Border terrier','courser','hound','hound dog','Plott hound','wolfhound','borzoi','Russian wolfhound','Irish wolfhound','Scottish deerhound','deerhound','coonhound','black-and-tan coonhound','coondog','foxhound', 
           'Walker hound','Walker foxhound','American foxhound','English foxhound','Weimaraner','otterhound','otter hound','bloodhound','sleuthhound','Norwegian elkhound','elkhound','Saluki','gazelle hound','Afghan hound','Afghan','staghound','greyhound','whippet',
           'Italian greyhound','beagle','harrier','basset','basset hound','bluetick','redbone','boarhound','Ibizan hound','Ibizan Podenco','Rhodesian ridgeback','dalmatian','coach dog','carriage dog','liver-spotted dalmatian','cur','mongrel','mutt','pariah dog','pye-dog','pie-dog',
           'feist','fice','corgi','Welsh corgi','Cardigan','Cardigan Welsh corgi','Pembroke','Pembroke Welsh corgi','Mexican hairless','lapdog','Newfoundland','Newfoundland dog','poodle','poodle dog','miniature poodle','large poodle','toy poodle','standard poodle', 
           'basenji','Leonberg','griffon','Brussels griffon','Belgian griffon','Brabancon griffon','pug','pug-dog','working dog','Great Dane','guide dog','Seeing Eye dog','shepherd dog','sheepdog','sheep dog','Old English sheepdog','bobtail','Bouvier des Flandres',
           'Bouviers des Flandres','Rottweiler','komondor','collie','Border collie','Belgian sheepdog','Belgian shepherd','malinois','groenendael','Shetland sheepdog','Shetland sheep dog','Shetland','kelpie','briard','German shepherd','German shepherd dog','German police dog','alsatian',
           'watchdog','guard dog','schipperke','pinscher','Doberman','Doberman pinscher','affenpinscher','monkey pinscher','monkey dog','miniature pinscher','kuvasz','housedog','attack dog','police dog','seizure-alert dog','sled dog','sledge dog','Siberian husky','malamute','malemute',
           'Alaskan malamute','boxer','bull mastiff','Eskimo dog','husky','Sennenhunde','EntleBucher','Bernese mountain dog','Greater Swiss Mountain dog','Appenzeller','Saint Bernard','St Bernard','mastiff','Tibetan mastiff','hearing dog','bulldog','English bulldog','French bulldog',
           'spitz','Pomeranian','keeshond','chow','chow chow','Samoyed','Samoyede','Great Pyrenees','toy dog','toy','toy spaniel','King Charles spaniel','papillon','English toy spaniel','Blenheim spaniel','Pekinese','Pekingese','Peke','Maltese dog','Maltese terrier','Maltese',
           'toy terrier','Japanese spaniel','Shih-Tzu','Chihuahua','puppy'],
'flower':['daisy','flowerpot','flower','florist','anemone','vase'] 
         }
    ...
    model = Inception()
    driver = init_driver()
    image_links = get_all_download_links(driver,'https://athulyashyju.github.io/orgcap.html')
    
    #             driver.quit()
    dirname = 'imgs'
    make_dir(dirname)
    download_images('https://athulyashyju.github.io/orgcap.html', dirname, image_links)
    
    # Path for a jpeg-image that is included in the downloaded data.
    import glob
    
    c = glob.glob('imgs/*.jpg')
    for i in range(0,9):
        image_path=c[i]
        print(image_path)
        d=image_path[5]
    ...
    # Print the scores and names for the top-10 predictions.
        model.print_scores(pred=pred, k=5)
